/**
 * POS 49 — narrower nav (~30%), denser sell cards, full captions + desc.
 */
...
assert.match(ver, /POS_BUILD\s*=\s*49\b/);
...
assert.match(css, /\.pos-sell-item-desc/);
assert.doesNotMatch(css, /\.pos-sell-item-caption[\s\S]{0,200}line-clamp:\s*2/);

assert.match(sell, /item\.description/);
assert.match(sell, /pos-sell-item-desc/);
