...
assert.ok(Number(buildMatch[1]) >= 761, `APP_BUILD >= 761, got ${buildMatch[1]}`);
...
assert.match(panel, /cash-in-bill-card|is-tick/);
assert.match(panel, /โอนนำเข้า|เงินสดรอนำเข้า|รอโอน/);
assert.match(panel, /cash-in-slim/);
assert.match(panel, /cash-in-summary-slips/);
assert.match(panel, /\+สลิป/);
assert.match(panel, /เข้าบช\./);
assert.match(panel, /cash-in-remain/);
assert.match(panel, /cash-in-summary-bar/);
assert.match(panel, /cash-in-action-btn/);
assert.match(panel, /ยอดรวม|โอนเงินตามยอดนี้|ควรเข้า/);
...
assert.match(panel, /transferRows|summarizeCashDepositTransfers/);
assert.match(panel, /cash-in-history-summary/);
assert.match(panel, /วันโอน/);
assert.match(panel, /คชจ\./);
assert.doesNotMatch(panel, /flatRows/);
...
assert.match(panel, /cash-in-compact-btn|cash-in-action-btn/);
assert.match(lib, /suggestedNetBankTransfer/);
assert.match(lib, /export function summarizeCashDepositTransfers/);
...
assert.match(css, /\.cash-in-history-summary\b/);
assert.match(css, /\.cash-in-bill-card\.is-tick|\.cash-in-bill-check/);
