...
อ่านจากรูปแล้วดึงยอดที่โอนเข้าบัญชีร้าน
...
กฎ:
- bankAmount = ยอดเงินที่เข้าบัญชีผู้รับ (ไม่รวม fee ถ้าแยก) ตัวเลขไม่มี comma
- transferFee = ค่าธรรมเนียมโอน ถ้าสลิปแสดง 0 หรือไม่มีให้เป็น 0
- ถ้ามี 1–2 รูปของรายการโอนเดียวกัน ให้รวมเป็นหนึ่งรายการ (อย่าบวกยอดซ้ำ)
- transferDate = วันที่บนสลิปโอน
- ห้ามแต่งข้อมูลที่มองไม่เห็นในรูป`;

const DAY_SYSTEM_PROMPT = `คุณเป็นผู้ช่วยอ่านสลิปสรุปยอดขาย POS / nPOS / mPOS ของร้านเครื่องดื่มไทย
เป้าหมายคือดึงยอดเงินสดที่ต้องนำส่ง (ไม่ใช่ยอดขายรวมทั้งหมด)

ตอบเป็น JSON เท่านั้น:
{"date":"YYYY-MM-DD หรือว่าง","cashAmount":ยอดเงินสดหรือ null,"drawerCloseAmount":ยอดปิดลิ้นชัก/เงินสดที่นับได้หรือ null,"slipKind":"daily|shift|unknown","shiftLabel":"เช้า|เย็น|ว่าง","reason":"เหตุผลสั้นๆ ภาษาไทยไม่เกิน 40 ตัวอักษร"}

กฎ:
- cashAmount = ยอด "เงินสด" / Cash จากสรุปการชำระเงิน (Payment) — สำคัญที่สุด
- อย่าใช้ยอดขายรวม (Total Sales) ถ้ามีแยกเงินสด
- drawerCloseAmount = Expected Cash / Actual Cash / เงินปิดลิ้นชัก ถ้ามีในสลิป ไม่งั้น null
- date = วันที่ของรายงานบนสลิป (อาจเป็น พ.ศ. ให้แปลงเป็น ค.ศ. YYYY-MM-DD)
