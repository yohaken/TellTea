...
/**
 * Bulk edit option price deltas (storefront) across all groups.
 */
export function PosMenuOptionPriceTable({
  ...
  function getDraft(r: Row) {
    const key = rowKey(r);
    return draft[key] ?? String(r.choice.priceDelta ?? 0);
  }

  function setCell(r: Row, value: string) {
    const key = rowKey(r);
    ...
    try {
      ...
      for (const groupId of dirtyGroupIds) {
        ...
        const options = group.options.map((c) => {
          ...
          return {
            ...c,
            priceDelta: Math.max(0, Number(raw) || 0),
          };
        });
  ...
  return (
    <div className="pos-menu-price-table-wrap">
      <p className="muted pos-menu-sort-hint">
        ตั้งราคาเพิ่มของตัวเลือก (หน้าร้าน) · ราคาแอปส่งตั้งที่จัดการราคาช่องทาง · ไม่มี CSV
      </p>
                ...
                <tr key={rowKey(r)}>
                  ...
                  <td>
                    <input
                      ...
                      step={0.01}
                      value={store}
                      onChange={(e) => setCell(r, e.target.value)}
