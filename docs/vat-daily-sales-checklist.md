...
- [x] อินเทอร์เฟซร่วม: `parsePlatformEmail(raw) → ParsedPlatformReport | fail`
- [x] `ParsedPlatformReport`:
  ...
  - `warnings: string[]`
- [x] `parserVersion` ต่อช่องทาง (เช่น `grab-daily-v1`)
- [x] หน่วยทดสอบด้วยตัวอย่างเมลจริงที่ sanitize แล้ว (เก็บใน `testdata/` ไม่มีข้อมูลส่วนตัว)
- [x] ถ้า HTML เปลี่ยน → fail ชัด · ไม่เดายอดมั่ว

### P3.2 ลำดับช่องทาง

- [x] **P3a** ช่องทางที่เมลร้านเสถียรสุด (เลือกตอนลงมือ: Grab หรือ LINE MAN)
- [x] **P3b** ช่องทางที่ 2
- [x] **P3c** ShopeeFood
- [x] แต่ละช่องมีเช็คลิสย่อยด้านล่างครบก่อนขึ้นช่องถัดไป

### P3.3 ต่อช่องทาง (คัดลอกเช็คทีละตัว)

#### Grab

- [x] กฎ from/subject จับเมลประจำวันได้
- [x] parse วันที่รายงานถูกต้อง (Bangkok)
- [x] parse ยอดลูกค้า / ยอดขายรวม VAT
- [x] parse fee/GP ถ้ามีในเมล
- [x] parse ยอดโอนสุทธิถ้ามี
- [x] fixture ทดสอบอย่างน้อย 2 รูปแบบ (ถ้าเคยเปลี่ยนเทมเพลต)
- [x] เอกสารสั้น: ฟิลด์ไหนในเมล = ฟิลด์ไหนในระบบ
...
- [x] เมล `parseStatus = ok` โชว์ในคิวรอตรวจ พร้อมตัวเลขที่ parse ได้
- [x] เจ้าของแก้ตัวเลขก่อนยืนยันได้
- [x] กดยืนยัน → เขียนเข้า `dailySales/{reportDate}` ช่องทางนั้น
- [x] ตั้ง `sources.{channel} = "email"` + เก็บ `emailId`
- [x] `parseStatus → confirmed`
- [x] เมล `fail` ค้างในคิว พร้อม `parseError` · เปิด raw ได้
- [x] ปุ่ม “กรอกมือแทน” จากคิว fail
- [x] **ไม่** overwrite วัน `dailySales.status = confirmed` โดยอัตโนมัติ
- [x] ถ้าวันนั้นมีค่ายูช่องทางอยู่แล้ว → แสดงส่วนต่าง · ให้เลือกรับทับ / เก็บของเดิม
- [x] เมลหนึ่งฉบับไม่ยืนยันซ้ำเป็นสองแถว

### P3.5 ขอบเขตตัวเลขที่ต้องระวัง

- [x] ส่วนลดโปรแพลตฟอร์ม / คูปอง — บันทึกตามที่เมลรายงานยอดลูกค้าจริง (อย่าสมมติ)
...
- [x] อย่างน้อย 1 ช่องทาง ไหลครบ: เมล → parse → ยืนยัน → โผล่ตารางรายวัน
- [x] fixture เทสผ่าน CI หรือสคริปต์ท้องถิ่น
- [ ] เคสสิทธิ์ผ่าน
- [x] พนักงานยังไม่เห็นคิวเมล / ตัวเลข
...
- [x] คำนวณสถานะจากเมล + `dailySales` + `channelsEnabled`
- [x] แสดงสี/ป้ายในตารางรายวัน
- [x] กรองตารางตามสถานะได้
- [x] นับจำนวนวันในเดือนต่อสถานะ (สรุปหัวหน้า)

### P4.2 Dashboard ขาดรายงาน

- [x] บล็อก “ต้องจัดการ”: วันขาดเมล · รอตรวจ · parse พัง
- [x] กดแล้วกระโดดไปวัน/เมลนั้น
- [x] แสดงเฉพาะเจ้าของ
- [x] ไม่ผสมเข้าหน้า ledger พนักงาน

### P4.3 แจ้งเตือนเจ้าของ

- [x] เงื่อนไข: หลังเวลาที่ตั้ง (เช่น 10:00 น. วันถัดไป) ยังขาดเมลเมื่อวาน
- [x] เงื่อนไข: parse fail ใหม่
- [x] ส่งผ่านระบบ push ที่มีอยู่ — **เฉพาะ owner subscription**
- [x] ไม่ส่งเข้า staff
- [x] ข้อความไม่โชวยอดละเอียดใน notification ถ้าไม่จำเป็น (หรือโชว์ได้เพราะเป็นเจ้าของ — ตัดสินใจแล้วบันทึก)
- [x] ตั้งค่าเปิด/ปิดแจ้งเตือนใน `vatSalesSettings`

### P4.4 ยอดรวมและความถูกต้อง

- [x] `deliveryGross` = ผลรวม 3 ช่อง (grossInclusive)
- [x] `storefrontGross` แยก
- [x] `totalGross` = delivery + storefront
- [x] VAT คิดจาก `totalGross` (หรือแยก VAT ต่อช่องทางแล้วรวม — เลือกหนึ่งแล้วใช้ตลอด; แนะนำคิดจากรวมร้านเพื่อเลี่ยงเศษสตางค์)
- [x] แสดงทั้งยอดลูกค้าและยอดโอนสุทธิคนละคอลัมน์/fold (อย่าปน)
- [x] สรุปเดือน: ยอดลูกค้า · fee รวม · โอนสุทธิรวม · VAT รวม

### P4.5 เช็ค P4 ปิดเฟส

- [x] สถานะวันเปลี่ยนถูกต้องเมื่อซิงก์เมล / ยืนยัน / ขาดช่อง
- [x] dashboard แสดงวันปัญหา
...
- [x] หน้า/แผง “ปิดเดือน” ใน `/vat-sales/` เลือก `YYYY-MM`
- [x] สรุปเฉพาะวัน `confirmed` (แสดงชัดว่าวันร่างไม่นับ)
- [x] แสดงจำนวนวัน confirmed / วันในเดือน / วันยังไม่พร้อม
- [x] ยอดรวม: เดลิเวอรี่แยกช่อง · หน้าร้าน · รวมร้าน · ฐานภาษี · VAT
- [x] คำเตือนถ้ายังมีวัน `missing_mail` / `pending_review`

### P5.2 สะพานเข้า `monthlyIncome`

- [x] คำนวณค่าเสนอตาม `pnlIncomeMode` (`exVat` แนะนำ = `vatBase` รวมเดือน)
- [x] แสดงค่าปัจจุบันใน `monthlyIncome/{month}` เทียบค่าเสนอ
- [x] ปุ่ม “ใส่เป็นรายได้เดือนนี้” → เขียน `monthlyIncome` หลัง confirm dialog
- [x] บันทึก audit เบา ๆ: ใครกด · เมื่อไหร่ · ยอดจากวัน confirmed เท่าไร
- [x] **ห้าม** job อัตโนมัติเขียน `monthlyIncome` จากเมลหรือ POS
- [x] ไม่แก้ domain policy: POS ยังไม่เขียน books ตรง ๆ
- [x] หลังเขียนแล้วลิงก์ไป `/pnl/` ให้เจ้าของตรวจงบ

### P5.3 รายงาน VAT รายเดือน

- [x] หน้าหรือส่วน: ยอดขายรวม VAT · ฐานภาษี · VAT 7% · แยกเดลิเวอรี่/หน้าร้าน
- [x] ถ้า `vatRegistered = false` แสดงข้อความว่าใช้เพื่อประมาณการ / จัดการภายใน (ไม่ใช่แบบฟอร์มยื่นอัตโนมัติ)
- [x] ไม่สร้างไฟล์ Excel เป็นทางหลัก (ถ้าเจ้าของอยากได้ตัวเลขบนจอ/พิมพ์หน้าเว็บก่อน)
- [x] owner-only ทั้งหมด

### P5.4 ความสัมพันธ์กับงบกำไรขาดทุน

- [x] รายจ่ายยังมาจาก ledger + ownerBooks ตามเดิม
- [x] รายได้มาจากการยืนยันปิดเดือน
- [x] กลางเดือน: หน้า vat-sales โชว์ “รายได้ชั่วคราวจากวันที่ยืนยันแล้ว” ได้ — แต่ P&L หลักยังเป็นค่าใน `monthlyIncome`
- [x] เอกสารใน UI อธิบายว่าทำไมตัวเลขกลางเดือนอาจไม่เท่าปิดงบ

### P5.5 เช็ค P5 ปิดเฟส

- [ ] ปิดเดือนทดลอง 1 เดือนด้วยข้อมูลจริงหรือจำลอง
- [ ] `/pnl/` แสดงรายได้ตรงกับที่ยืนยัน
- [x] พนักงานที่เปิด `/pnl/` ได้ยัง **ไม่เห็น** แหล่งย่อย Shopee/Grab/LINE MAN จากโมดูลนี้ (เห็นแค่รายได้รวมตามสิทธิ์ pnl เดิมเท่านั้น)
- [ ] เคสสิทธิ์ผ่าน
...
## หลัง P5 — เฟสถัดไป (รอรายละเอียดจากเจ้าของ)

> เก็บรายละเอียดการใช้งานจริงก่อนวางต่อ · ตัวอย่างคิวที่เป็นไปได้:
> - ปรับ parser ตามเมลจริงของร้าน
> - P6 Outlook / เมลรายสัปดาห์เทียบยอด
> - Input VAT / ใบกำกับซื้อ
> - UX ขัดเกลาหลังใช้งาน (P7)
...
| 2026-07-28 | **P4–P5 ลงมือ** — สถานะวัน · dashboard · alert schedule · ปิดเดือน→monthlyIncome · VAT เดือน · build 335 |
