...
อัปเดต: **1.14.35** · ฟอนต์ Prompt + ปุ่มสั้นตาม mockup (ไม่ยืดเต็มจอ)
...
4. ปุ่มหลักสูง ~52dp · กว้าง **wrap_content** (max ~280dp) จัดกลาง · รอง/ghost ~40–44dp · งานย่อยเป็น **chip** — **ห้าม** `match_parent` / `maxWidth=999dp` ยืดเต็มจอ  
5. สีแบรนด์: ส้ม `#E85D24` · หมึก `#1A2E24` · พื้น `#F7F7F5` (`colors.xml`)
6. Programmatic CTA ใช้ `NposUi.cta()` ไม่ใช่ `matchWidth()` (ยกเว้นแถวรายการ)
