...
อัปเดต: **1.14.13** (ใบเสร็จ) · ship ปัจจุบัน **1.14.105** (พิมพ์เร็วท้องถิ่น) · `APP_BUILD` 548 · `POS_BUILD` 159 · `versionCode` 128  
...
- [x] รายการ: ชื่อฐานตัวหนา · ราคาบรรทัด · qty นำหน้าเสมอ (ASCII) · modifiers `- … xN` · เว้นบรรทัดคั่นรายการ
- [x] รวม: จำนวน · รวม · ส่วนลด · **ยอดสุทธิ** · ชำระ · เงินสด/ทอน
- [x] ฟุตเตอร์ `receiptFooterNote` ของร้านเท่านั้น · **ไม่มี** `TellTea POS`
...
### R3 จังหวะพิมพ์ (ไม่เสียกระดาษซ้ำ / ไม่ทิ้งลูกค้า) — **1.14.105** เร็วสุด
- [x] พิมพ์ (+เตะลิ้นชักสด) **ทันทีหลังบันทึกในเครื่อง** — ไม่รอ HTTP (`npos-print-fast-local`)
- [x] เลขบนกระดาษ: `#รอส่ง-XXXXXX` (web formatPendingBillNo) · ประวัติอัปเป็นเลขบิลจริงหลังซิงก์
- [x] ธง `printed` / `receiptPrinted` — `flushOne` ไม่พิมพ์ซ้ำหลังซิงก์สำเร็จ
- [x] flush / retry คิวเก่าที่ยังไม่พิมพ์: พิมพ์ครั้งเดียว (ได้เลขบิลจริงถ้าซิงก์ทัน)
...
- [ ] ส่วนลด > 0 โผล่บนสลิป
- [ ] option หลายตัว · xN เฉพาะเมื่อ ≥2 · x2+ ตัวหนา · ไม่มี `?` จาก ×/•
- [ ] ออฟไลน์สั้นๆ → ได้กระดาษเลข รอส่ง-… · ซิงก์ทีหลังไม่พิมพ์ซ้ำ
...
- [x] bold ESC/POS บนชื่อรายการ (+ x2+ option) — 1.14.80
- [ ] double-size ESC/POS บนหัวบิล (ถ้าเครื่องรองรับ)
