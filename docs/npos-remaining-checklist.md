...
อัปเดต: **1.14.38** + แผน **Exclusive seat S1–S4** · friction F0–F5  
...
- [x] **Doc + drawer** เอกสารร้าน (ชื่อร้านเรา · ไม่ลอกแบรนด์คู่แข่ง) · CashDrawerPolicy (**1.14.38**)
- [x] จิ้มเมนู → จ่าย → ใบเสร็จ → รีเซ็ต · ลิ้นชักตอนสด

## คิวถัดไป (เฟสก่อนหน้าที่ยังไม่ทำ)
| เฟส | โฟกัส | สถานะ | ทำไมค้าง |
|-----|--------|--------|----------|
| **Shop settings persist** | ชื่อ/ที่อยู่หลังบ้าน → Firebase → ใบเสร็จ | ✅ | **1.14.38** · แก้ local ทับ cloud + reload native |
| **Friction F0–F5** | เคลม UX · ลิ้นชัก≠พิมพ์ · กะออฟไลน์ · tip | ⬜ แผน | ดู `npos-ops-friction-phases.md` · รองจาก seat |
| **Doc + drawer** | เอกสารร้าน · ลิ้นชัก policy · ติดตั้งฮาร์ด | ✅ | **1.14.38** · ค้างคนเทสเคาน์เตอร์ |
...
- [x] **เข้างาน→ขายทันที** · dialog เหตุเตะ · HB ~5s + ชิป BO · จำรหัสร้าน · ปุ่มขายแถวยาว+ไอคอน (**1.14.38**)
- [x] **Compact buttons** wrap_content · max 280dp · ไม่ยืดเต็มจอ (**1.14.38**)
- [x] **Friendly UI sweep** NposUi ทั้งแอป + นโยบายฝังงานใหม่ · แถบซ้าย/จอลูกค้า/option (**1.14.38**)
- [x] **Kick bounce จากหน้าขาย** NposApp CLEAR_TOP เมื่อถูกเตะ + HB force ~5s + ชิปนับถอยหลัง (**1.14.38**)
- [x] **POS number pad** ทอนเปิดกะ · ปิดกะ · เคลม · เงินสด — คีย์สูง `padKeyMinPx` (**1.14.38**)
- [x] **บิลย้อนหลัง + หมวดเมนู** การ์ด Npos · ชิปหมวดใหญ่ขึ้น (**1.14.38**)
- [x] **Link status orb** เขียว/เหลือง/แดง + นับถอยหลัง BO (**1.14.38**)
- [x] **BO รหัสเต็ม + ตารางรอบ/ยอด** · แผนเก็บยอด `npos-bo-sales-retention-plan` (**1.14.38**)
- [x] **Kick-reclaim K1–K3** ปุ่มเตะในตาราง · เปลี่ยนรหัส = เตะทุกเครื่อง · client อัป hash + เด้งใส่รหัส (**1.14.38**) — ค้างคนเทส
- [x] **Exclusive seat S1–S4** เครื่องเดียว + ตาราง slim + เตะ + resume กะ + ด่านอัปเดต (**1.14.38**) — ค้างคนเทส
- [ ] **Friction F0–F5** คนเทส + เคลม UX + ลิ้นชัก≠พิมพ์ + กะออฟไลน์ — `npos-ops-friction-phases.md`
- [x] **Counter UX batch** แบดจ์จำนวน · นาฬิกากะ · ซ่อน QR · โลโก้ (**1.14.38**)
- [x] **Doc + drawer** เอกสารร้าน + ลิ้นชัก policy (**1.14.38**)
