...
อัปเดต: **แผน** (ยังไม่ ship โค้ด) · อ้างอิง `/pos-sales` + nPos 1.14.85  
...
## O1 — ใครเข้ากะ (เรียบง่าย)

**ปัญหา:** หลังร้านเห็นเครื่อง/รอบ แต่ไม่รู้คน  
**เป้า:** ตอนเปิดกะเลือกชื่อจากรายชื่อพนักงาน → บันทึกบน `posSessions` → โชว์ที่ `/pos-sales`  
**ไม่ทำ:** PIN · สิทธิ์แยกรายคน · ผูก OT อัตโนมัติ

เช็คลิส: [npos-shift-opener-checklist.md](./npos-shift-opener-checklist.md)

### งานย่อ
- [ ] O1.1 ดึงรายชื่อ `employees` (หรือ roster ที่ใช้จริง) ลงเครื่องตอนอุ่นข้อมูล
- [ ] O1.2 `OpenShiftFlow`: หลังกรอกทอน → เลือกชื่อ (บังคับ 1 คน)
- [ ] O1.3 `nposSessionOpen` + `ShiftPrefs` เก็บ `openedByEmployeeId` / `openedByName`
- [ ] O1.4 Z «โดย» ใช้ชื่อผู้เปิดกะ (fallback `receiptStaffName`)
- [ ] O1.5 `/pos-sales` คอลัมน์/ป้าย **ผู้เปิดกะ**
- [ ] O1.6 Gate + คนเทส
